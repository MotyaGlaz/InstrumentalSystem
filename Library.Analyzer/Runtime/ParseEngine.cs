...
namespace Library.Analyzer.Runtime
{
    public class ParseEngine : IParseEngine
    {
        ...
        public List<INormalState> PulseList(IReadOnlyList<IToken> tokens)
        {
            List<INormalState> normalStates = new List<INormalState>();
            for (var i = 0; i < tokens.Count; i++)
                normalStates.AddRange(ScanPassList(Location, tokens[i]));
            ...
            _nodeSet.Clear();
            return CleanStateList(normalStates);
        }

        private List<INormalState> CleanStateList(List<INormalState> list)
        {
            var result = new List<INormalState>();
        ...
        private List<INormalState> ScanPassList(int location, IToken token)
        {
            List<INormalState> normalStates = new List<INormalState>();
            var earleySet = _chart.EarleySets[location];
            for (int s = 0; s < earleySet.Scans.Count; s++)
            {
                var scanState = earleySet.Scans[s];
                normalStates.Add(ScanList(scanState, location, token));
            }
            return normalStates;
        }

        private INormalState ScanList(INormalState scan, int j, IToken token)
        {
            var i = scan.Origin;
