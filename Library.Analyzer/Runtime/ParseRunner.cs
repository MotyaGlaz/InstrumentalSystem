...
namespace Library.Analyzer.Runtime
{
    public class ParseRunner : IParseRunner
    {
        ...
        public List<(ILexeme, List<INormalState>)> Lexemes { get { return _listOfTokens; } }

        public ParseRunner(IParseEngine parseEngine, string input, ConsoleWriter writer = null)
            : this(parseEngine, new StringReader(input), writer)
        {
        }

        public ParseRunner(IParseEngine parseEngine, string input, ParseRunnerOptions options, ConsoleWriter writer = null)
            : this(parseEngine, new StringReader(input), options, writer)
        {
        }

        public ParseRunner(IParseEngine parseEngine, TextReader reader, ConsoleWriter writer = null)
            : this(parseEngine, reader, new ParseRunnerOptions(), writer)
        { 
        }

        public ParseRunner(IParseEngine parseEngine, TextReader reader, ParseRunnerOptions options, ConsoleWriter writer = null)
        {
            ParseEngine = parseEngine;
            Options = options;
            _listOfTokens = new List<(ILexeme, List<INormalState>)>();
            _reader = reader;
        ...
        private bool HandleError()
        {
            ...
            if (!(_writer is null))
            {
                StringBuilder builder = new StringBuilder($"В строке {Line + 1} в столбце {Column} ожидался один из токенов:");
                var set = new HashSet<ILexerRule>(expectedLexerRules);
                foreach (var error in set) 
                {
                    builder.Append(" " + error.ToString());
                }
                _writer.WriteError(builder.ToString());
            }
        ...
        private bool TryParseExistingToken()
        {
            ...
            if (ParseEngine is ParseEngine engine)
            {
                var rulesList = engine.PulseList(_tokenLexemes);
                if (rulesList is null)
                    return false;
                if (rulesList.Count == 0)
                    return false;
                _listOfTokens.Add(new(_tokenLexemes.ElementAt(0), rulesList));
            }
            else
            {
            ...
            {
                //if (_previousTokenLexemes[0].LexerRule.TokenType.Id.Equals("SORT"))
                //    if (_tokenLexemes[0] is DfaLexeme dfaLexeme)
                //       Console.WriteLine($"Main name -> {dfaLexeme.Capture}");
